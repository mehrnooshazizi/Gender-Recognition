...
for x in Male:
    ...
    for (x,y,w,h) in faces:
        newframe = cv2.rectangle(gray,(x,y),(x + w, y + h),(0,255,0),2)           
...
model=Sequential()
model.add(Conv1D(96,7,activation='relu',padding='same',input_shape=(img.shape[1],img.shape[2])))
...
model.add(BatchNormalization())
model.add(Dropout(0.5))
model.add(Dense(2,activation='softmax',activity_regularizer=L2(0.005)))
model.summary()
...
net=model.fit(x_train,y_train,batch_size=258,epochs=10,verbose=1,validation_split=0.1)
